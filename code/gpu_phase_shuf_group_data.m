function shuf_data = gpu_phase_shuf_group_data(real_data)
...
%transform the vectors-to-be-scrambled to the frequency domain and the
%real_data to GPU array
real_data_gpu = gpuArray(real_data);
Fx = fft(real_data_gpu,[],1);
clear real_data 
...
clear rand_phase
z = gpyArray(x_amp.*exp(J.*sym_phase)); %generate (symmetric)-phase-scrambled Fourier components
clear sym_phase
